...
// Check if the request is for adding or updating
if (isset($_POST['courseid'])) {
    ...
    try {
        ...
        if ($_FILES["certificate"]["size"] > 1000000 || $_FILES["transcript"]["size"] > 1000000) {
            header("location:../academic.php?r=2290");
            exit();
        }

        $stmt = $conn->prepare("INSERT INTO tbl_academic_qualification (member_no, country, institution, course, level, timeframe, certificate, transcript) 
                                VALUES (:member, :country, :institution, :course, :level, :timeframe, :certificate, :transcript)");
